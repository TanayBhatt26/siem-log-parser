...
Bug fixes applied:
  #1  — RICH=False + binary .evtx: events was set to None, causing TypeError downstream.
  #12 — watch: open() inside while True leaked one fd per tick (~1800/hour at 2s interval).
  #13 — query: open(dest,"wb").write(...) relied on GC to close; now uses with statement.
  #14 — watch: detect_format() called on each new chunk; Zeek #fields headers are only
          in the file header, not in chunks, so mid-file chunks got misdetected as syslog.
          Format is now detected once from the file header before the loop begins.
"""
...
def _print_stats(events, detected, filename):
    ...
    if RICH:
        ...
        geo_count = sum(1 for e in events if getattr(e, 'geo_country', None))
        mal_count = sum(1 for e in events if getattr(e, 'is_malicious', False))
...
def _render_table(events, limit=50, show_geo=False):
    base_cols = ["timestamp","severity","source_format","source_host","source_ip",
    ...
    if not RICH:
        for evt in events[:limit]:
            ...
            click.echo(f"  {str(d.get('timestamp',''))[:19]}  {str(d.get('severity','')):8}  "
                       f"{str(d.get('source_ip','')):16}  {str(d.get('message',''))[:60]}")
        return
    t = Table(box=box.MINIMAL_DOUBLE_HEAD, header_style="bold cyan",
...
@click.option("-o","--output","output_fmt", default="json",
...
@click.option("--enrich",  "do_enrich",  is_flag=True, help="Add GeoIP + threat intel")
...
@click.option("--abuseipdb-key", default="", help="AbuseIPDB API key")
def parse_cmd(file, input_fmt, output_fmt, dest, index, limit, no_preview,
    ...
    # Bug #1 fix: RICH=False path set events=None for binary files, crashing everything
    # downstream (_print_stats, export, etc.). Always parse regardless of RICH.
    if is_binary and fmt == "evtx":
        ...
        if RICH:
            with Progress(SpinnerColumn(), TextColumn("[cyan]{task.description}"), transient=True) as p:
                p.add_task(f"Parsing binary EVTX: {os.path.basename(file)}...")
                events = parse_evtx_file(file)
        else:
            click.echo(f"Parsing binary EVTX: {file}...")
            events = parse_evtx_file(file)
    else:
        if RICH:
            with Progress(SpinnerColumn(), TextColumn("[cyan]{task.description}"), transient=True) as p:
    ...
    if do_store:
        ...
        if RICH:
            console.print(f"[green]✓ Stored {n} events[/green] (session: [cyan]{sid}[/cyan])")
        else:
            click.echo(f"Stored {n} events (session: {sid})")
    ...
    # Bug #13 fix: use with statement to guarantee file handle is closed
    with open(dest, "wb") as fh:
        fh.write(result)
    if RICH:
        console.print(f"\n[bold green]✓ Exported[/bold green] → [underline cyan]{dest}[/underline cyan]  "
                      f"[dim]({len(result):,} bytes)[/dim]")
    else:
    ...
    if RICH:
        if ips:
            ...
            for ip, cnt in ips.most_common(5):
                geo = next((f" ([dim]{e.geo_country}[/dim])"
                            for e in events if e.source_ip == ip and e.geo_country), "")
...
def query(severity, source_ip, username, country, search, malicious, session, limit, output_fmt):
    ...
    if RICH:
        console.print(f"[bold]Found {result['total']} events[/bold]  "
                      f"[dim](showing {len(events)})[/dim]\n")
    if output_fmt:
        ...
        # Bug #13 fix: with statement guarantees handle is closed
        with open(dest, "wb") as fh:
...
def db_stats():
    ...
    s = get_stats()
    if RICH:
        t = Table(box=box.ROUNDED, title="[bold]Database Statistics[/bold]",
        ...
        if s.get("sessions"):
            ...
            for sess in s["sessions"][:5]:
                console.print(f"  [dim]{sess['created_at'][:19]}[/dim]  "
                              f"[cyan]{sess['session_id']}[/cyan]  "
                              f"{sess['filename']} ({sess['event_count']} events)")
    else:
...
def watch(file, input_fmt, interval, do_enrich):
    ...
    _banner()
    if RICH:
        console.print(f"[bold cyan]Watching[/bold cyan] {file} — Ctrl+C to stop\n")
    else:
        click.echo(f"Watching {file} — Ctrl+C to stop")

    # Bug #14 fix: detect format once from the file header, not on each chunk.
    # Zeek logs keep #fields only in the header — new chunks look like plain data
    # rows and get misdetected as syslog without the header context.
    if input_fmt == "auto":
        with open(file, errors="replace") as fh:
            header_sample = fh.read(8192)
        cached_fmt = detect_format(header_sample)
    else:
        cached_fmt = input_fmt

    seen = 0
    try:
        while True:
            # Bug #12 fix: use with statement so the fd is closed each iteration.
            # open() inside while True without close() leaks ~1800 fds/hour at 2s interval.
            with open(file, errors="replace") as fh:
                lines = fh.readlines()
            ...
            if new:
                ...
                for evt in evts:
                    ...
                    geo = f" [{evt.geo_country}]" if getattr(evt, 'geo_country', None) else ""
                    if RICH:
                        color = SEV_COLORS.get(sev, "white")
                        mal   = " [red]⚠ MALICIOUS[/red]" if getattr(evt, 'is_malicious', False) else ""
                        console.print(
                            f"[dim]{ts}[/dim]  [{color}]{SEV_ICONS.get(sev,'')} {sev.upper():<8}[/{color}]  "
                            f"[cyan]{str(evt.source_ip or ''):16}[/cyan]{geo}{mal}  "
                            f"{str(evt.message or '')[:60]}"
                        )
                    else:
                        click.echo(f"{ts}  {sev.upper():<8}  {str(evt.source_ip or ''):16}{geo}  "
                                   f"{str(evt.message or '')[:60]}")
            time.sleep(interval)
